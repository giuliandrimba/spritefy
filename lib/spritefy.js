...
(function() {
  ...
  __t('app.build').SpriteBuilder = (function() {
    ...
    SpriteBuilder.prototype.build = function(images_path) {
      ...
      cmd = "convert " + this.list_files + " +append " + this.sprite_path + "/" + this.name + ".png";
  ...
  __t('app.utils').Files = (function() {
    ...
    Files.prototype.setup_images = function(callback) {
      var count, image, _i, _len, _ref, _results,
        _this = this;
      count = 0;
      _ref = this.images;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        image = _ref[_i];
        _results.push(easyimage.info(image.file, function(err, file) {
          if (err) {
            throw err;
          }
