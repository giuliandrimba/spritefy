...
(function() {
  var easyimage, fs, path, program, util,
  ...
  __t('app.build').PluginBuilder = (function() {
    var copyFileSync;

    PluginBuilder.prototype.plugin_file = path.join(path.dirname(fs.realpathSync(__filename)), '../spritefy-animation/jquery.spritefy.js');

    PluginBuilder.prototype.plugin_file_min = path.join(path.dirname(fs.realpathSync(__filename)), '../spritefy-animation/jquery.spritefy.min.js');

    PluginBuilder.prototype.new_path = path.resolve("scripts/");

    function PluginBuilder() {}

    PluginBuilder.prototype.build = function() {
      if (fs.existsSync(this.new_path) === false) {
        fs.mkdirSync(this.new_path);
      }
      copyFileSync(this.plugin_file, "" + this.new_path + "/jquery.spritefy.js");
      return copyFileSync(this.plugin_file_min, "" + this.new_path + "/jquery.spritefy.min.js");
    };
  ...
  __t('app.build').SpriteBuilder = (function() {
    ...
    SpriteBuilder.prototype.build = function(images_path, callback) {
      var cmd,
        _this = this;
      this.sprite_path = path.resolve("sprites/");
      if (fs.existsSync("" + this.sprite_path + "/" + this.name + ".png")) {
  ...
  __t('app').Main = (function() {
    ...
    Main.prototype.generate = function(folder, name, min, quality) {
      this.folder = folder;
      this.name = name;
      this.min = min;
      ...
      this.images_filter.bind("complete", this.generate_sprite);
      return this.generate_jquery_plugin();
    };
    ...
    Main.prototype.generate_jquery_plugin = function() {
      this.plugin = new app.build.PluginBuilder;
